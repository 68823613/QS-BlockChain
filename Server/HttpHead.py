...
class HttpRequest(object):
    ...
    def passRequest(self,bc,request):
        ...
        if self.method == 'POST':
            ...
            # request_data 是传过来的数据
            # print "dataid:",dataid
            # print "dataact:",dataact
            self.action(dataid,dataact,bc)
            # write code xxxxxx
            # if datas["type"] == 'user-edit':
            #     print "user-edit!"
    ...
    def action(i,dataid,dataact,bc):#增加修改相应
        dataid = dataid.split('>',1)
        # print "action",dataact
        # print "dataid",str(dataid)
        if dataact == 'user-add' or dataact == 'doctor-add':
            print "add"
            dataid = ''.join(dataid)
            block_chain.adddata(dataid,dataact,bc)
        elif dataact == 'user-edit' or dataact == 'doctor-edit':
            dataid = ''.join(dataid)
            block_chain.editdata(dataid,dataact,bc)
        else:
            print "action error"
